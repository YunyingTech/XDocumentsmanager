...
本文档描述 XDocuments `0.1.1` 的实际技术状态、关键数据链路和后续演进顺序。路线以本地优先、可恢复和固定资源占用为原则，面向超大规模 PDF 目录；“60TB”是设计目标，仍需通过长期基准测试和真实数据集持续验证。
...
| PaddleOCR | 本机 | Python Worker 单任务串行处理，支持进度和取消 | 当前仍依赖系统 Python/在线安装环境 |
...
- **软件托管 PaddleOCR 运行时**：按操作系统和架构发布固定版本的 Python/Paddle 运行时包；首次使用时下载到应用数据目录，校验 SHA-256，临时目录解压并原子切换；提供下载、安装、验证、失败清理和重试进度。普通用户不再配置 Python 路径，高级设置可保留自定义解释器入口。
...
P0 完成标准：全新 Windows 安装无需预装 Python 即可启用 PaddleOCR；中途退出不会丢失任务状态；文本型 PDF 无需 OCR 即可全文搜索；超大 PDF 预览不再一次加载完整文件。
