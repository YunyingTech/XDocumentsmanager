...
自动化结果：`34 passed`。`pytest --cov=agent --cov=core --cov=tools` 的语句覆盖率为 `85%`（1885 条语句，276 条未覆盖）。
...
| C10 | Streamlit AppTest 启动应用 | 页面无异常并显示六个工作区 | 0 exception；入库/问答/图片点评/质量/对比/参考文献标签均存在 | 通过 |
| C11 | SHA-256 `8089ae8f...d62ee39` 的 arXiv 2607.28702v1 双栏 PDF | 原生结构不完整时自动 MineU OCR，HITL 非空并可入库检索 | 识别 7 个核心章节及 Acknowledgments；确认后 41 分片；检索返回章节/页码/段落来源；缓存命中 0.29s | 通过 |
| C12 | 2607.28702v1 的 MineU `content_list.json` 和图片目录 | 确认后展示全部结构化图片，并保留图注/章节/邻文 | 16/16 个 JPG 可读取（11 chart、2 image、3 formula）；浏览器实际渲染 451×309 图片且无页面异常 | 通过 |
...
| E9 | DeepSeek 接口拒绝图片输入 | 自动降级，不暴露供应商异常 | 真实接口切换为 `text_only`，依据 MineU 图注、章节和相邻正文生成点评并标注证据模式 | 通过 |

## 结论

核心链路、HITL、多轮隔离、长期记忆、流式输出、质量子 Agent、图片点评和异常降级均有测试覆盖。真实论文章节准确率定义为“正确识别的预期核心章节类别数 / 预期核心章节类别总数”，两篇实测均为 100%。
